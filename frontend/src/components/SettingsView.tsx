      ...
      items: [
        {
          ...
          control: (
            <div className="flex gap-2">
              <button onClick={() => setDedupMode('auto')} className={`px-3 py-1.5 text-[10px] font-semibold border tracking-normal ${dedupMode === 'auto' ? 'bg-black text-white border-black shadow-[2px_2px_0px_#3b82f6]' : 'bg-white text-slate-500 border-slate-300 hover:border-black'}`}>
                Auto
              </button>
              <button onClick={() => setDedupMode('manual')} className={`px-3 py-1.5 text-[10px] font-semibold border tracking-normal ${dedupMode === 'manual' ? 'bg-black text-white border-black shadow-[2px_2px_0px_#3b82f6]' : 'bg-white text-slate-500 border-slate-300 hover:border-black'}`}>
                Manual
              </button>
            </div>
          ),
        },
        {
          ...
          control: (
            <div className="flex items-center gap-3">
              ...
              <span className="text-[13px] font-bold text-black w-6">{throttleLimit}</span>
            </div>
          ),
        },
      ],
  ...
  return (
    <div className="space-y-6 max-w-3xl mx-auto pb-12">
      <div className="flex items-center gap-3">
        ...
        <h2 className="font-black text-black text-lg tracking-normal">Settings</h2>
      </div>
          ...
          return (
            <div key={section.title} className="bg-white border-2 border-black shadow-[4px_4px_0px_rgba(0,0,0,1)] overflow-hidden">
              <div className="bg-black text-white px-6 py-3 flex items-center gap-3">
                ...
                <h3 className="font-semibold text-[11px] tracking-normal">{section.title}</h3>
              </div>
              <div className="divide-y divide-black">
                {section.items.map((item) => (
                  <div key={item.label} className="px-6 py-5 flex items-center justify-between gap-6">
                    <div className="min-w-0">
                      <h4 className="font-extrabold text-[13px] text-black ">{item.label}</h4>
                      <p className="text-[10px] text-slate-500 font-bold mt-0.5">{item.description}</p>
                    </div>
      ...
      <div className="bg-amber-50 border-2 border-black p-5 shadow-[4px_4px_0px_rgba(0,0,0,1)] flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h4 className="font-extrabold text-black text-[12px] ">Cache & Local Data</h4>
            <p className="text-[10px] text-slate-600 font-bold">File index cache, search vectors, and session data.</p>
          </div>
        </div>
        <button onClick={() => alert('Cache cleared.')} className="px-4 py-2 bg-white border border-black text-slate-700 hover:bg-red-50 hover:text-red-600 text-[10px] font-semibold tracking-normal shadow-[2px_2px_0px_rgba(0,0,0,1)] active:translate-y-0.5 transition-all flex items-center gap-1.5">
